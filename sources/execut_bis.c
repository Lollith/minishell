...
/*   Updated: 2022/08/23 16:02:30 by agouet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fd_monitor(t_pipe *pipex)
{
	if (pipex->pipefd[pipex->act_p][0] && (pipex->ctrl == 0))
		ft_link_fd(pipex->pipefd[pipex->act_p][0], pipex->pipefd[pipex->act_p][1], STDOUT_FILENO);
	if (pipex->pipefd[pipex->act_p][0] && pipex->ctrl == 1)
	{
		ft_link_fd(pipex->pipefd[pipex->act_p + 1][1], pipex->pipefd[pipex->act_p +1][0], STDIN_FILENO);
		ft_link_fd(pipex->pipefd[pipex->act_p][0], pipex->pipefd[pipex->act_p][1], STDOUT_FILENO);
	}
	if (pipex->nb_pipes > 1 && pipex->pipefd[pipex->act_p][0] && pipex->ctrl == -1)
		ft_link_fd(pipex->pipefd[pipex->act_p + 1][1], pipex->pipefd[pipex->act_p+1][0], STDIN_FILENO);
	else
		ft_link_fd(pipex->pipefd[pipex->act_p][1], pipex->pipefd[pipex->act_p][0], STDIN_FILENO);
}
